...
class CustomBoarOverlay extends Overlay {
	...
	private final CustomBoarConfig config;
	...
	@Inject
	private CustomBoarOverlay(final Client client, final CustomBoarConfig config, final ItemManager itemManager){
	...
	private void playSound(String name){
		...
		if(currentTime - soundTime < 4000)
			return;
		try {
			rt.exec("ffplay -nodisp -autoexit -volume "+config.volume()+" /home/boar/.osrs/"+name+".mp3");
			soundTime = currentTime;
		} catch(Exception e){}
	}

	private void superCombatPotion(Graphics2D graphics){
		ImageComponent component = new ImageComponent(itemManager.getImage(ItemID.SUPER_COMBAT_POTION4));
